...
## 完整例子：Android 知识库助手的 `/api/chat`

假设贯穿项目要做“Android 崩溃日志智能分析”，第一版可以把 `/api/chat` 设计成一个稳定网关入口，由后端判断是否需要 RAG、工具调用或降级。
...

{
  ...
  "conversation_id": "conv-crash-001",
  "scenario": "android_crash_analysis",
  "message": "这段 FATAL EXCEPTION 是什么原因？",
  "attachments": [
    {
      "type": "text",
      "name": "crash.log",
      "content": "java.lang.NullPointerException at MainActivity.onCreate"
    }
  ],
  "stream": true
}
...
后端不要把供应商事件原样透传给 Android，而是转换成自己的事件协议：

```text
event: message.started
data: {"trace_id":"tr_01","message_id":"asst_01"}

event: message.delta
data: {"message_id":"asst_01","text":"这个崩溃首先看 NullPointerException。"}

event: message.citation
data: {"label":"[crash-guide#2]","title":"Android 空指针排查","sourceUri":"app://knowledge/android/null-pointer"}

event: message.completed
data: {"message_id":"asst_01","usage":{"input_tokens":980,"output_tokens":220}}
```

如果触发业务边界，也要返回结构化错误，方便 Android 映射成稳定 UI：
...
```json
{
  ...
  "message": "当前分析请求较多，请稍后再试。",
  "retryable": true,
  "retry_after_seconds": 30
}
```

这个例子的重点不是接口名，而是让一次 AI 请求有清楚的输入、事件、错误、引用和 trace。后续换模型或改 RAG，不应该迫使 Android 重写状态机。
