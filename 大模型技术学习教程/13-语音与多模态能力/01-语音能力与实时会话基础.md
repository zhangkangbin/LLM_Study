# 语音能力与实时会话基础

语音能力可以拆成三类：语音转文字、文字转语音、实时语音会话。它们看起来都和“声音”有关，但工程形态完全不同。

## 三种语音能力

| 能力 | 输入 | 输出 | 适合场景 | 工程特点 |
| --- | --- | --- | --- | --- |
| 语音转文字 | 音频文件或音频流 | 文本 | 会议纪要、语音搜索、录音整理 | 可以异步，适合走普通 HTTP 上传 |
| 文字转语音 | 文本 | 音频 | 播报、陪练、无障碍朗读 | 可以缓存，适合后端生成后返回 |
| 实时语音会话 | 麦克风音频 | 文本/音频事件 | 语音助手、陪练、低延迟问答 | 需要长连接、打断、重连和状态同步 |

普通语音助手可以先从“录音 -> 转写 -> 文本问答 -> 语音播报”开始。只有当你需要用户边说边打断、模型边听边回答、延迟接近通话体验时，才需要 Realtime。

## 普通语音链路

```mermaid
sequenceDiagram
    participant User as 用户
    participant App as Android App
    participant API as 业务后端
    participant STT as 语音转文字
    participant LLM as 文本/RAG/Agent
    participant TTS as 文字转语音

    User->>App: 按住说话
    App->>App: 录音、降噪、压缩
    App->>API: 上传音频
    API->>STT: 转写
    STT-->>API: 文本
    API->>LLM: 问答、检索或工具调用
    LLM-->>API: 文本回答
    API->>TTS: 生成语音
    TTS-->>API: 音频
    API-->>App: 文本 + 音频地址/字节流
    App-->>User: 展示文字并播放语音
```

这条链路的优点是简单、稳定、好排查。它可以复用前面阶段的后端网关、RAG、Agent、结构化输出和日志系统。

## 实时语音链路

实时语音不是把普通 HTTP 请求变快一点，而是一次持续会话。客户端持续采集音频，服务端持续返回事件，模型可以发出文本、音频、状态、错误和工具调用事件。

```mermaid
flowchart LR
    Mic["麦克风帧"] --> Client["Android Realtime Client"]
    Client --> Session["短时会话凭证"]
    Session --> Realtime["Realtime API"]
    Realtime --> EventBus["音频 / 文本 / 状态事件"]
    EventBus --> Player["播放器"]
    EventBus --> UI["字幕与状态"]
    UI --> Interrupt["用户打断"]
    Interrupt --> Realtime
```

实时语音要重点处理：

1. 会话凭证：Android 不保存模型密钥，只从后端拿短时凭证或会话配置。
2. 音频格式：客户端和服务端要约定采样率、编码、分片大小。
3. VAD：由客户端或服务端判断用户是否说完。
4. 打断：用户再次说话时，要停止当前播报并通知会话取消或截断。
5. 重连：网络切换、App 进后台、蓝牙设备切换都可能让会话中断。
6. 事件顺序：文本字幕、音频播放和最终回答状态可能不是同一时间到达。

## Android 端建议

先不要一上来做全双工实时语音。更稳妥的路线是：

1. 第一版：按住说话，上传完整录音，返回文本回答。
2. 第二版：在第一版基础上增加 TTS 播报。
3. 第三版：增加分片上传或流式转写，降低等待时间。
4. 第四版：再接 Realtime，实现打断、低延迟和持续会话。

这样做的好处是每一步都有清晰的可测边界。即使后面接入实时会话，前面的 STT/TTS 链路仍然可以作为降级方案。

## 实战例子：用语音提问崩溃日志

Android 知识库助手可以先做一个普通语音入口：

```text
用户按住说话
-> App 录音 8 秒并显示波形
-> App 上传音频到 /api/ai/speech/transcribe
-> 后端转写出“这个空指针为什么发生”
-> 后端把转写文本交给 /api/chat
-> RAG 检索 Android 空指针排查文档
-> 返回文字答案和引用
-> 可选调用 /api/ai/speech/speak 生成播报音频
```

这个版本不需要全双工实时语音，但已经覆盖了录音、上传、转写、RAG、TTS、取消和重试。用户取消时，Android 立即停止录音或播放；后端如果已经开始模型调用，也要在 trace 里记录 `user_cancelled`。

## Realtime 取舍例子

什么时候值得接 Realtime？可以用这个表判断：

| 需求 | 普通 STT/TTS | Realtime |
| --- | --- | --- |
| 用户按住说一句，等回答 | 适合 | 没必要 |
| 会议录音转摘要 | 适合 | 没必要 |
| 用户想随时打断播报 | 勉强可做 | 更适合 |
| 类似电话一样连续对话 | 不适合 | 适合 |
| 弱网环境下稳定提交任务 | 更稳 | 更容易断线 |

贯穿项目第一版建议先用普通 STT/TTS。只有当产品明确需要“边说边听、随时打断、低延迟陪练式对话”时，再把 Realtime 作为增强能力接入。

## 后端接口草图

```http
POST /api/ai/speech/transcribe
Authorization: Bearer <app-session-token>
Content-Type: multipart/form-data

file=<audio>
```

```http
POST /api/ai/speech/speak
Authorization: Bearer <app-session-token>
Content-Type: application/json

{
  "text": "请用自然语气朗读这段回答"
}
```

```http
POST /api/ai/realtime/session
Authorization: Bearer <app-session-token>
Content-Type: application/json

{
  "scenario": "voice_assistant",
  "voice": "marin"
}
```

真实项目中，这些接口都应该由你的业务后端实现。Android 只拿业务 token，不拿模型供应商 API Key。
